...
class Twoot_Generator {
	...
	*/
	function logo() {
		$logo = twoot_get_frontend_func('opt', 'opt', 'standard_logo');
	...
	*/
	function responsive_menu_switch() {
		$html = '<div class="responsive-menu-switch-wrap">';
		$html .= '<a href="#" class="responsive-menu-switch icon open" id="toggle-top-responsive-menu"></a>';
		$html .= '</div>';
	...
	*/
	function social_icons($position, $show_icons) {
		...
		$all_icons = array('behance', 'dribbble', 'deviantart', 'facebook', 'flickr', 'fivehundredpx', 'gplus', 'linkedin', 'instagram', 'pinterest', 'rss', 'soundcloud', 'tumblr', 'twitter', 'vimeo', 'youtube');
				...
				foreach($current_icons as $current_icon)
				{
					if(in_array($current_icon, $all_icons, true))
					{
						if($icons[$current_icon]) {
							$html .= '<a href="'.$icons[$current_icon].'" class="'.$current_icon.'" rel="external"><i class="icon icon-'.$current_icon.'"></i></a>';
						}
					}
				}
			}
	...
	*/
	function icons() {
		$icons = array(
			...
			'deviantart' => twoot_get_frontend_func('opt', 'opt', 'deviantart'),
			...
			'gplus' => twoot_get_frontend_func('opt', 'opt', 'gplus'),
	...
	*/
	function search() {
		if(twoot_get_frontend_func('opt', 'opt', 'show_search')) {
			...
			$html .= '<a class="top-search-bt open" href="#"><i class="icon"></i></a>';
			$html .= '</div>';
	...
	*/
	function top_text() {
		$phone_number = twoot_get_frontend_func('opt', 'opt', 'phone_number');
		$work_time = twoot_get_frontend_func('opt', 'opt', 'work_time');
		$phone_icon = twoot_get_frontend_func('opt', 'opt', 'phone_icon');
		$time_icon = twoot_get_frontend_func('opt', 'opt', 'time_icon');
		$phone_icon = $phone_icon? '<i class="icon '.$phone_icon.'"></i>':'';
		$time_icon = $time_icon? '<i class="icon '.$time_icon.'"></i>':'';

		if($phone_number || $work_time) {
			$html = '<div class="top-text">';
			if($phone_number) { 
				$html .= '<span class="phone-number">'.$phone_icon.stripslashes($phone_number).'</span>';
			}
			if($work_time) { 
				$html .= '<span class="work-time">'.$time_icon.stripslashes($work_time).'</span>';
			}
			$html .= '</div>';
	...
	*/
	function page_title($type) {
		switch ($type) 
		{
			case 'page':	
				...
				$text_color = twoot_get_frontend_func('meta', 'page_desc_text_color');
				$bg_color = twoot_get_frontend_func('meta', 'page_desc_bg_color');
				$bg_image = twoot_get_frontend_func('meta', 'page_desc_bg_image');
				$bg_repeat = twoot_get_frontend_func('meta', 'page_desc_bg_repeat');
				$bg_horizontal = twoot_get_frontend_func('meta', 'page_desc_bg_horizontal');
				$bg_vertical = twoot_get_frontend_func('meta', 'page_desc_bg_vertical');
			break;

			case 'portfolio':
				$title = get_the_title();
				$desc = twoot_get_frontend_func('meta', 'page_desc')==false? get_the_excerpt():twoot_get_frontend_func('meta', 'page_desc');
				$text_color = twoot_get_frontend_func('meta', 'page_desc_text_color');
				$bg_color = twoot_get_frontend_func('meta', 'page_desc_bg_color');
				$bg_image = twoot_get_frontend_func('meta', 'page_desc_bg_image');
				$bg_repeat = twoot_get_frontend_func('meta', 'page_desc_bg_repeat');
			...
			case 'archive':
				$desc = $text_color = $bg_color = $bg_image = $bg_repeat = $bg_horizontal = $bg_vertical = '';
				if ( is_category() )
				{
					$args = array ('<p>', '</p>');
				...
				elseif (is_search())
				{
					switch (twoot_get_frontend_func('opt', 'opt', 'search_type')) 
					{
						case '1': $types=array('post', 'page', 'portfolio'); break;
				...
				elseif (is_tag())
				{
					...
					$desc = str_replace(array ('<p>', '</p>'), '', term_description());
				}
				elseif(is_tax())
				{
				...
				elseif (is_404())
				{
					$title = esc_attr__('404 Page','Twoot');
				}
				else
				{
		...
		$html = '<header class="site-page-header">';
		$html .= '<section class="entry-header">';
		$html .= '<div class="container">';
		$html .= '<div class="inner clearfix">';
		$html .= '<h1 class="entry-title"><i class="icon icon-menu"></i>'.$title.'</h1>';
		ob_start();
		new twoot_breadcrumbs();
		$html .= ob_get_clean();
		$html .= '</div>';
		$html .= '</div>';
		$html .= '</section>';
		if($desc) {
			$html .= '<section class="entry-desc"'.$this->page_title_style($text_color, $bg_color, $bg_image, $bg_repeat, $bg_horizontal, $bg_vertical).'>';
			$html .= '<div class="container">';
			$html .= '<div class="inner">'.stripslashes($desc).'</div>'; 
			$html .= '</div>';
			$html .= '</section>';
		}
		$html .= '</header>';
	 ...
	 * Page Title
	*/
	function page_title_style($text_color, $bg_color, $bg_image, $bg_repeat, $bg_horizontal, $bg_vertical) {

		$css = $text_color? 'color:'.$text_color.';':'';
		$css .= $bg_color? 'background-color:'.$bg_color.';':'';
		if($bg_image) {
			$css .= $bg_image? 'background-image:url('.$bg_image.');':'';
	...
	function post_slider($ids, $width, $height, $crop, $lightbox) {
		if(is_array($ids) && !empty($ids))
		{
			...
			foreach ($ids as $id) {
				...
				$caption=stripslashes(get_post($id)->post_excerpt);
				$alt=strip_tags(get_post_meta($id, '_wp_attachment_image_alt', true));
				$tilte=$alt? ' title="'.$alt.'"':'';
				$class=is_singular(array( 'post', 'portfolio' ))? 'class="fancybox-slider"':'class="not-fancybox-gallery"';
				...
				{
					$html .= twoot_get_frontend_func('resize_thumbnail', $id, $alt, $width, $height, $crop).'<div class="overlay"></div>';
				}
				else
				{
					$html .= '<a href="'.$link.'" '.$class.$rel.$tilte.'>'.twoot_get_frontend_func('resize_thumbnail', $id, $alt, $width, $height, $crop).'<div class="overlay"></div></a>';
				}
	...
	*/
	function portfolio_meta() {
		...
		if($client_url=twoot_get_frontend_func('meta', 'client_url')) {
			$html .= '<div class="client-url"><a href="'.$client_url.'" class="button button-active">'.esc_attr__('Launch Project &raquo;', 'Twoot').'</a></div>'; 
		}
		$html .= '</div>';
	...
	*/
	function contact_form() {
		...
		$show_form = twoot_get_frontend_func('opt', 'opt', 'show_quick_contact');
		$contact_form_id = twoot_get_frontend_func('opt', 'opt', 'quick_contact_form');
		$contact_form_name=twoot_get_frontend_func('page_name', $contact_form_id);

		if($show_form && $contact_form_id) {

			$html = '<div class="quick-contact-wrapper">';
			$html .= '<a href="#" class="switch" id="quick-contact-switch"></a>';
			$html .= '<div class="quick-contact hide">';
			$html .= '<h3>'.esc_attr__('Contact US', 'Twoot').'</h3>';
			$html .= do_shortcode('[contact-form-7 id="'.$contact_form_id.'" title="'.$contact_form_name.'"]');
			$html .= '</div>';
			$html .= '</div>';
