...
?>
<?php get_header(); ?>
<?php echo twoot_generator('page_title', 'portfolio'); ?>
...
<article id="primary-wrapper" class="twelve clearfix">
	...
	<?php 
		switch($type)
		{
			case 'slider': echo twoot_generator('post_slider', $gallery_id, 1170, 650, true, true); break;
			case 'image': echo twoot_generator('post_image', $gallery_id, 1170, 650, true, true); break;
			case 'masonry': echo twoot_generator('post_masonry', $gallery_id, 580, 9999, false, true, $columns); break;
			case 'video': echo twoot_generator('post_video', $video_type, $video_url, $video_embed, 1170, null); break;
		}
	?>
	</div>
	<div class="eight column full">
		<div class="inner full-content">
		<div class="entry-content entry-text"><?php the_content(); ?></div>
		</div>
	</div>
	<div class="four column full">
		...
		<nav class="post-pagenation">
			<ul class="clearfix">
			<?php if( get_previous_post() ) : ?><li class="prev"><?php previous_post_link('%link', __( '<i class="icon-left-open"></i>', 'Twoot' )) ?></li><?php endif; ?>
			<?php if( $portfolio_page_id = twoot_get_frontend_func('opt', 'opt', 'portfolio_page_id') ) : ?><li class="back"><a href="<?php echo get_page_link($portfolio_page_id); ?>"><i class="icon-th"></i></a></li><?php endif; ?>
			<?php if( get_next_post() ) : ?><li class="next"><?php next_post_link('%link', __( '<i class="icon-right-open"></i>', 'Twoot' )) ?></li><?php endif; ?>
			</ul>
		</nav>
		<h1 class="entry-title item-title"><?php the_title(); ?></h1>
	...
	<div class="eight column half">
		...
		<?php
			switch($type)
			{
				case 'slider': echo twoot_generator('post_slider', $gallery_id, 770, 500, true, true); break;
				case 'image': echo twoot_generator('post_image', $gallery_id, 770, 500, true, true); break;
				case 'masonry': echo twoot_generator('post_masonry', $gallery_id, 580, 9999, false, true, $columns); break;
				case 'video': echo twoot_generator('post_video', $video_type, $video_url, $video_embed, 770, null); break;
			}
		?>
		</div>
	</div>
