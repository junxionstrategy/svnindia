...
class Twoot {
	...
	*/
	public function supports() {
		...
		if(function_exists('add_theme_support')) {
			...
			add_theme_support('post-thumbnails', array('post', 'portfolio', 'testimonial'));
	...
	*/
	function includes() {
		...
		if(twoot_get_checked_func('wpml_activated')) {
			require_once( TWOOT_PATH . '/theme/wpml/wpml.php' );
		}
	}
	...
	*/
	public function admin_includes() {
		...
		require_once( TWOOT_PATH . '/theme/metaboxes/meta-page-desc.php' );
		require_once( TWOOT_PATH . '/theme/metaboxes/meta-testimonial.php' );
	...
	*/
	public function frontend_includes() {
		...
		include_once( TWOOT_PATH . '/theme/classes/class-breadcrumb.php' );
		...
		include_once( TWOOT_PATH . '/theme/classes/class-template-list.php' );
		include_once( TWOOT_PATH . '/theme/classes/class-template-related-posts.php' );
		include_once( TWOOT_PATH . '/theme/classes/class-template-faq.php' );
	}
	...
	*/
	function styles() {
		...
		wp_register_style('js-composer-animate', $this->theme_url(). '/css/js-composer-animate.css', false, TWOOT_VERSION, 'all');
		wp_register_style('js-composer-front', $this->theme_url(). '/css/js-composer-front.css', false, TWOOT_VERSION, 'all');
		wp_register_style('icon-fonts', $this->theme_url(). '/css/icon-fonts.css', false, TWOOT_VERSION, 'all');
		...
		if ( ! empty( $post->post_content ) && strstr( $post->post_content, '[vc_row' ) ) {
			wp_enqueue_style('js-composer-animate');
	...
	*/
	function scripts() {
		...
		wp_register_script('twoot-animation', $this->theme_url(). '/js/jquery.animation.js', false, '1.0.0', true);
		...
		if( ! empty( $post->post_content ) && ( strstr( $post->post_content, '[progress_bar' ) || strstr( $post->post_content, '[number' ) ) ) {
			wp_enqueue_script('twoot-appear');
			wp_enqueue_script('twoot-countTo');
		}
		if( ! empty( $post->post_content ) && ( strstr( $post->post_content, '[tabs]' ) || strstr( $post->post_content, '[left_tabs]' ) || strstr( $post->post_content, '[vc_tabs]' ) || strstr( $post->post_content, '[vc_tour]' ) ) ) {
		...
		if ( ! empty( $post->post_content ) && strstr( $post->post_content, '[vc_row' ) ) {
			wp_enqueue_script('twoot-animation');
		}
		if( twoot_get_checked_func('IE_check') ) {
		...
		if( is_page_template('template-contact.php') || ( ! empty( $post->post_content ) && strstr( $post->post_content, '[gmap' ) ) ) {
	...
	*/
	function add_inline_style() {
		if( is_tax('portfolio_cat') || is_tax('portfolio_tag') || is_singular('portfolio') || is_tag() || is_category() || is_singular('post') ) {
			...
			$css = '#top-menu ul li'.$selecter.'-item-'.$id.' a { color: '.$link_color.'; }';
			$css .= '#top-menu ul li'.$selecter.'-item-'.$id.' a:hover, #top-menu ul li'.$selecter.'-item-'.$id.'.selected a { color: '.$hover_color.'; }';
...
class Twoot_Admin {
	...
	*/
	public function menus() {
		add_theme_page( esc_attr__('Theme Options',  'Twoot'), esc_attr__('Theme Options',  'Twoot'), 'edit_theme_options', 'theme_opt', array($this,'theme_opt') );
		add_theme_page( esc_attr__('Theme Skins',  'Twoot'), esc_attr__('Theme Skins',  'Twoot'), 'manage_options', 'theme_skin', array($this,'theme_skin') );
		add_theme_page( esc_attr__('Theme Fonts',  'Twoot'), esc_attr__('Theme Fonts',  'Twoot'), 'manage_options', 'theme_font', array($this,'theme_font') );
	}
	...
	*/
	public function activated() {
		if ( ! twoot_get_checked_func('theme_setup') && 'themes.php' == basename($_SERVER['PHP_SELF']) && isset($_GET['activated']) && $_GET['activated']=='true' ) {
			...
			// Update meta
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_id' ), '_wp_page_template', 'template-builder.php');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_id' ), TWOOT_PREFIX . 'layout', 'full');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v2_id' ), '_wp_page_template', 'template-builder.php');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v2_id' ), TWOOT_PREFIX . 'layout', 'full');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v3_id' ), '_wp_page_template', 'template-builder.php');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v3_id' ), TWOOT_PREFIX . 'layout', 'full');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v4_id' ), '_wp_page_template', 'template-builder.php');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_home_v4_id' ), TWOOT_PREFIX . 'layout', 'full');
			...
			update_post_meta(get_option( TWOOT_PREFIX . 'template_contact_id' ), TWOOT_PREFIX . 'layout', 'right');
			...
			update_post_meta(get_option( TWOOT_PREFIX . 'template_faq_id' ), '_wp_page_template', 'template-faq.php');
			update_post_meta(get_option( TWOOT_PREFIX . 'template_faq_id' ), TWOOT_PREFIX . 'layout', 'full');
